...
## Planning Meetings (RPM.md, sprintN.md) (max 5 marks)
  ...
  - RPM.md (Release Planning Meetings) (max 3 marks)
    ...
    Feedback: You presented a simple project overview as release goal, which is really a vague representation of release goals.
    You should specify what new features users should be able to use after the next release.
    (e.g., register as a user, buy products, create product sell post, review products.)
...
## User Stories (Tracked in Trello) (max 12 marks) 
  ...
  - Prioritized	(max 4 marks)
    ...
    Your Mark: 2 (Some of the registration and profile viewing tasks do not have priorities) 
  ...
  - Clearly Defined Acceptance Criteria (max 2 marks)
    ...
    Your Mark: 2 (Ideally each card should have an acceptance criteria because each
    card is a testable unit.) 
...
## Tracking on Trello  (max 15 marks) 
  ...
  - Do the pull request on GitHub match the tasks listed on Trello Active Sprint Board? (max 2 marks)
    ...
    The pull requests do not have any meaningful title, description,
    or Trello link that can be used to distinguish pull requests according to
    tasks (without reading the full code change in that PR, which defeats the
    purpose of task tracking systems). 
  ...
  - Is each pull request made from a separate branch for each individual task? (max 2 marks)
    ...
    Your Mark: 1 (Some pull requests seemed to have multiple tasks (like doc update
    in the same feature pull request.))
  ...
  - Is the git commit message clear? (max 2 marks)
    ...
    Your Mark: 1 (A lot of the commit messages are default (like 'Update <filename>'))
...
Tracking on Trello Total Mark: 11 / 15
...
Total Mark for Sprint Completion: 10 / 10
...
## System Design (PDF or MD file or another suitable format) (max 10 marks) 
  ...
  - CRC Cards [or equivalent, if the team is not using CRC) (max 4 marks)
    ...
    Your Mark: 1 (Does not provide responsibilities for any class.)
...
Total mark on System Design: 2/10
...
## Sprint Video Demo (Max 12 marks) 
  ...
  - Presentation (max 2 marks)
    ...
    Your Mark: 1/2 (UI is bare minimum)
...
Total Mark for sprint video: 9 / 12
