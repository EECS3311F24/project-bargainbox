...
## Daily Stand-ups (max 10 marks)
  ...
  Daily Stand-ups Total Mark: 8 / 10 (All standup updates posted just before the deadline,
  and also in a single commit which makes it impossible to know if each memeber updated
  their part.  Standup updates ideally should be recorded after the standup so that you can refer
  to that for future work or next standups. Otherwise what's the point?)
...
## Tracking on Trello  (max 24 marks) 
  ...
  - Branch Naming (max 2 marks)
    ...
    Your Mark: 0 (Branch name == member name)
